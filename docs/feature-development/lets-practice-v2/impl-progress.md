...
**Status:** 15 of 16 steps complete (counting 9a + 9b + 9c). Destructive frontend pass: ExamReviewPage deleted; `/exam-review/:sessionId`, `/exam/:sessionId`, old `/practice/:sessionId` routes removed; ExamReview types + `getExamReview` + `endExamEarly` + `endPracticeSession` + `ExamSummary` + `EndPracticeResponse` + `Turn.exam_*` + `CreateSessionRequest` exam/practice modes + `GuidelineSessionEntry` exam/practice fields + `ReportCardTopic.latest_exam_*` + `TopicProgress.last_practiced` + `ResumableSession` practice mode + TutorWS state.exam_progress all stripped from api.ts; ReportCardPage legacy exam chip dropped; ChatSession had ~700 LOC of exam/practice chat-mode dead code (hydrateExamState, handleSubmitAllExamAnswers, handleEndPractice, exam-progress header, practiceComplete card, exam-summary in completion, full exam-mode view) removed and Teach Me → Practice CTA rewired to navigate to `/practice/:guidelineId` drill route; SessionHistoryPage exam filter pill + exam badge + exam score render dropped; 170 lines of `.exam-review-*` CSS (both default + chalkboard) stripped; `api-exam-lifecycle.test.ts` deleted. Grep-gate `(exam-review|end-exam|/exam/|end-practice|ExamReviewPage|getExamReview|latest_exam_score|latest_exam_total)` returns 0. `npm run build` clean (1076 modules, -1 from ExamReviewPage delete). Next and last step is Step 14 — docs.
...
## Next step briefing — Step 14

**Goal:** Docs. The code is done; the docs need to catch up. Per plan §14.

**Files to create:**
- `docs/principles/practice-mode.md` — the "why" of the batch-drill model (12 formats, half-point rounding, bank regen doesn't orphan history via snapshot isolation, etc.).
- `docs/functional/practice-mode.md` — student-facing functional flow (Practice tile gating, landing, runner, results, banner, Reteach).
- `docs/technical/practice-mode.md` — backend+frontend architecture (schema, atomic submit, ThreadPool grader, capture component layer, banner polling, AuthenticatedLayout).

**Files to update:**
- `docs/principles/scorecard.md` — practice chip replaces exam chip; update the metric list and rationale.
- `docs/technical/architecture-overview.md` — add practice services/repos + runtime path + Practice v2 routes.
- `docs/technical/database.md` — document `practice_questions` + `practice_attempts` tables + the partial unique index.
- `docs/technical/ai-agent-files.md` — practice_bank_generator + practice_grader entries.
- `CLAUDE.md` doc index table — add the three new practice docs.

**Files to delete:**
- `docs/feature-development/teach-me-practice-split/` — superseded by this feature. The old PRD described a different split of the old chat-mode practice + exam; all of that is gone. Remove the directory.

**Success criteria:**
- `/update-all-docs` skill run at end, clean.
- Every `exam`/`practice chat-mode` reference in docs either removed or clearly marked as historical.
- `docs/principles/practice-mode.md` exists, covers all the Locked design decisions from this impl-progress doc.
- No broken internal doc links.

**What to read first:**
- The PRD (`prd.md`) + impl-plan (`tech-impl-plan.md`) + this doc — all three are source material for the new docs.
- `docs/DOCUMENTATION_GUIDELINES.md` — repo doc style guide (concise, per memory `feedback_concise_docs.md`).
- Sibling principle/functional/technical docs (e.g. `learning-session.md`) — match the tone and depth.
...
- Commits on the branch (oldest → newest):
  ...
  - (step 13 — destructive frontend cleanup — committed after this update)
