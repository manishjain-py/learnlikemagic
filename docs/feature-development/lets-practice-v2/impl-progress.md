...
**Status:** 14 of 16 steps complete (counting 9a + 9b + 9c). Destructive backend pass: exam_service + exam_prompts + practice_prompts + test_exam_lifecycle deleted; exam + chat-practice branches removed from orchestrator, session_service, master_tutor, session_state, report_card_service, session_repository; /end-exam + /exam-review + /end-practice endpoints gone; `sessions.exam_score` + `sessions.exam_total` columns dropped in a transactional DB migration alongside a DELETE of 36 legacy exam/practice session rows + 115 child event rows. Grep-gate returns 0 across source files. Next is Step 13 — destructive frontend cleanup (matching ExamReview page + `/exam/:id` routes + `latest_exam_score` reads).
...
## Next step briefing — Step 13

**Goal:** Destructive cleanup — frontend. Mirror Step 12's removals on the React side. Delete the `ExamReviewPage`, the `/exam/:sessionId` + `/exam-review/:sessionId` + old `/practice/:sessionId` routes, the `getExamReview` API client function, the `ExamReviewResponse`/`ExamReviewQuestion` TypeScript types, and the legacy `latest_exam_score` / `latest_exam_total` reads in `ReportCardPage` + `api.ts`. Per plan §13.

**Files to touch:**
- `llm-frontend/src/pages/ExamReviewPage.tsx` — delete.
- `llm-frontend/src/App.tsx` — delete `/exam/:sessionId`, `/exam-review/:sessionId`, old `/practice/:sessionId` routes + their imports.
- `llm-frontend/src/api.ts` — delete `ExamReviewResponse`, `ExamReviewQuestion`, `getExamReview`. Drop `latest_exam_score` + `latest_exam_total` optional fields from `ReportCardTopic` type.
- `llm-frontend/src/pages/ReportCardPage.tsx` — remove the legacy exam chip + its conditional render.
- Check `ModeSelectPage` / `ModeSelection` for any remaining `exam` / `practice` mode refs (Step 10 should have removed most).

**Grep-gate** (CI assertion to add in a follow-up):
```bash
grep -rE '(exam-review|end-exam|/exam/|end-practice|ExamReviewPage|getExamReview|latest_exam_score|latest_exam_total)' llm-frontend/src/
```
must return 0 after the pass.

**What to read first:**
- `App.tsx` — see what routes reference ExamReviewPage today.
- `api.ts` — locate the ExamReview types to delete.
- `ReportCardPage.tsx` — the exam chip renders alongside the practice chip from Step 11; drop the exam-chip JSX + its type guard.

**Success criteria:**
- `npm run build` clean.
- Grep-gate returns 0.
- ReportCardPage renders only the practice chip per topic (no legacy exam chip).
- No dead routes; 404 on `/exam/:id` and `/exam-review/:id` (they no longer match any `<Route>`).
...
- Commits on the branch (oldest → newest):
  ...
  - (step 12 — destructive backend + DB cleanup — committed after this update)
