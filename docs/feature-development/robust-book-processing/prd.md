...
| Decision | Rationale |
|----------|-----------|
| **Background threads (not Celery/SQS)** | This is a single-tenant admin tool running on App Runner. Adding message queue infrastructure (Redis, SQS, Celery workers) would be significant operational overhead for a tool used by one admin at a time. Threading + database-tracked progress is the right level of complexity. The job lock table prevents concurrent processing. |
| **Database-tracked progress (not in-memory)** | Progress must survive page reloads. The `book_jobs` table already exists — we enhance it rather than adding new infrastructure. If the container restarts mid-job, the job will be in `running` state with `last_completed_item` set, and the admin can resume. |
...
| **S3 upload first, OCR second** | S3 uploads are fast (~200ms per image). OCR is slow (~10s per page). By separating them, we get all images stored durably in seconds, then OCR runs at its own pace. If OCR fails, the images are safe and OCR can be retried. |
| **Per-page error granularity** | A bulk "failed" status isn't useful. Knowing exactly which page failed and why lets the admin make an informed decision: wait and retry (rate limit), fix the image (corrupt file), or skip it. |
| **Keep single-page upload** | Bulk upload is for initial book loading. Single-page upload remains useful for corrections, replacements, or adding pages after initial upload. |
