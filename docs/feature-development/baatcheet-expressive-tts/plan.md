...
- **Voice cast (locked by audition).** Mohan Sir = `Sekhar — Warm & Energetic` (`81uXfTrZ08xcmV31Rvrb`); Meera = `Amara — Calm & Intellectual Narrator` (`IEBxKtmsE9KTrXUwNazR`). Both Indian-English from the ElevenLabs shared library. ElevenLabs prohibits actual child voices industry-wide (CSAM/scam/consent risk), so Meera is an adult voice performing a youthful character — same approach as animation studios.
