...
After a chapter is synced, every topic enters a 10-stage post-sync pipeline. Each stage produces a different artifact the student session uses at runtime. Stages run independently per topic and the admin can trigger any stage individually, run them in dependency order from a chosen starting point, or run everything not-done in one click.

**The 10 stages and their order:**
