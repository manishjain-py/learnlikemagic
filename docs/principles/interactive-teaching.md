...
## 1. Interaction Is the Default

Kids engage when they DO things, not when they read things. Passive content is a bridge between active moments — never the destination. Every 2-3 cards of content should hand control back to the student: a tap, a sort, a choice, a prediction. When in doubt, add more interaction, not more explanation. Students who say "I want more questions" are telling you the ratio is off.

## 2. Explain Before Testing
...
## 4. Scaffolded Correction (3-Stage)
...
## 12. Prefer Structured Input Options
...
## 13. Check-In Cards (Pre-Computed)

Lightweight interactive activities inserted between explanation cards during the card phase. They are comprehension checks — warm readiness signals, not quizzes. Kids love interaction — check-ins are where real engagement lives.

### Frequency — Paired Check-Ins
A **pair** of check-ins after every 2-3 content cards. Each pair = one **light** activity (~5-10s, recall) followed by one **heavy** activity (~15-25s, analysis). Both test the same preceding 2-3 cards but at different cognitive levels.

### Activity Types (11 formats, in two groups)

**Light types (~5-10s, recall/understand):**
...
- **odd_one_out** (~10s): Show 3-4 items, tap the one that doesn't belong. Best for classification, number properties.

**Heavy types (~15-25s, analyze/evaluate):**
...
- Mix in newer types — do not only use pick_one/true_false/fill_blank for the light slot.
- Use at least 4 different types across a variant.

### Placement Rules
- Never before card 3 (student needs content first).
- Never after summary card.
- Check-ins come in pairs (light + heavy) at the same card position.
- At least 2 content cards between different check-in pairs.
- Each pair tests ONLY concepts from the preceding 2-3 cards.
