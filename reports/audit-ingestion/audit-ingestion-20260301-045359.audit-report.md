...
1. **[CRITICAL] Fix broken stability logic** — `_check_and_mark_stable_subtopics` never updates index status. Topics accumulate indefinitely in context pack. **Estimated effort: S** (uncomment/re-implement 1 line + save index)

2. **[CRITICAL] Increase merge token limit** — `max_tokens=1500` in `GuidelineMergeService` silently truncates accumulated guidelines. Increase to 3000+ and add post-merge length validation. **Estimated effort: S**

3. **[CRITICAL] Implement V2 quality gates** — Add minimum quality checks: guidelines length > 100 chars, word count ratio vs. page count, empty guidelines detection. **Estimated effort: M**

4. **[HIGH] Increase boundary detection token limit** — `max_tokens=1000` constrains guidelines extraction. Increase to 2000. **Estimated effort: S**

5. **[HIGH] Increase dedup preview length** — `guidelines[:200]` is too short for accurate dedup. Increase to 500-800 chars. **Estimated effort: S**

6. **[HIGH] Fix DB sync atomicity** — Remove per-row `commit()` from `_insert_guideline`/`_update_guideline`. Single commit in `sync_book_guidelines`. **Estimated effort: S**

7. **[HIGH] Add post-extraction coverage report** — After extraction, compare page_index pages against total book pages. Report uncovered pages. **Estimated effort: S**

8. **[HIGH] Filter context pack by status** — Only include open/recent subtopics in boundary detection context, not all subtopics. **Estimated effort: M**

9. **[HIGH] Add non-content page handling** — Update boundary detection prompt to handle TOC, index, blank pages. **Estimated effort: S**

10. **[HIGH] Add guidelines extraction quality check** — Validate extracted guidelines are not empty, not too short, contain educational content. **Estimated effort: M**

11. **[MEDIUM] Increase context pack guidelines preview** — `guidelines[:300]` to 500-800 chars for better boundary matching. **Estimated effort: S**

12. **[MEDIUM] Add few-shot examples to boundary detection prompt** — Show examples of CONTINUE vs NEW decisions. **Estimated effort: M**

13. **[MEDIUM] Fix study plan orphaning on re-sync** — Add CASCADE delete or cleanup orphaned study plans before re-sync. **Estimated effort: S**
