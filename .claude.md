...
|-------------|----------------|---------|
| Infrastructure (Terraform, AWS) | `docs/technical/deployment.md` | Infrastructure Components, Deployment Process |
| API endpoints, database schema | `docs/technical/deployment.md` + `docs/technical/dev-workflow.md` | Making Changes |
| GitHub Actions workflows | `docs/technical/dev-workflow.md` | Deployment section |
| Local development setup | `docs/technical/dev-workflow.md` | Development Environment Setup |
| Troubleshooting solutions | Both documents | Troubleshooting sections |
| Build process (Makefile, Docker) | `docs/technical/dev-workflow.md` | Making Changes → Backend |
| Environment variables | `docs/technical/deployment.md` | Infrastructure Components |
...
# 4. Update docs/technical/dev-workflow.md → "Adding a New API Endpoint" example
...
# 4. Run migration in production (manual step - see docs/technical/deployment.md)
# 5. Update docs/technical/deployment.md → "Database Management" section
# 6. Commit & push
```
...
# 4. Update docs/technical/deployment.md → "Infrastructure Components" section
...
3. Consult `docs/technical/deployment.md` → "Troubleshooting" section
4. Verify architecture: `docker inspect IMAGE --format='{{.Architecture}}'`

**If you find a new issue/solution:**
- Document it in `docs/technical/deployment.md` and/or `docs/technical/dev-workflow.md`
...
```bash
# Backend (REQUIRED METHOD - Use venv, not Docker)
cd llm-backend
...
**⚠️ IMPORTANT:** Always run the backend using the local virtual environment with uvicorn, not Docker. Docker containers can mask import/dependency issues that cause admin endpoints to fail with 500 errors.
...
- [ ] Backend: `curl http://localhost:8000/admin/books` returns JSON (not 500 error)
- [ ] Backend: `curl http://localhost:8000/curriculum?country=India&board=CBSE&grade=3` returns subjects
...
|-------|-------------|---------------|
| Deployment fails | GitHub Actions logs → Architecture | `docs/technical/deployment.md` → Troubleshooting #1 |
| Health check fails | CloudWatch logs → Environment vars | `docs/technical/deployment.md` → Troubleshooting #1 |
| No logs in CloudWatch | Container not starting → Architecture! | `docs/technical/deployment.md` → Troubleshooting #2 |
| GitHub workflow doesn't trigger | Path filter in `.github/workflows/` | `docs/technical/dev-workflow.md` → Troubleshooting #4 |
| Local backend won't start | .env file → Dependencies → Python version | `docs/technical/dev-workflow.md` → Troubleshooting #1 |
| Frontend API errors | VITE_API_URL → CORS → Backend running | `docs/technical/dev-workflow.md` → Troubleshooting #2 |
...
1. **Check docs first:** Look in `docs/technical/deployment.md` or `docs/technical/dev-workflow.md`
...
- Update `docs/technical/dev-workflow.md` for new development patterns
- Update `docs/technical/deployment.md` for infrastructure changes
...
### DO:
✅ Always read `docs/technical/deployment.md` and `docs/technical/dev-workflow.md` before major changes
