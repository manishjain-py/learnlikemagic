...
python scripts/send_coverage_report.py \
  ...
  --subject "Unit Test Report — $BRANCH — $(date +%Y-%m-%d)" \
  --report "$SUMMARY_HTML" \
  --log "$LOG_FILE"
EMAIL_EXIT=$?
```
...
If both fail, clearly print local paths for manual share.
