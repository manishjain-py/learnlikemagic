...
## Step 2b: Generate human-readable test case docs

Create markdown files under `docs/test-cases/` — one file per suite — from `scenarios.json`.
...
root = Path(os.environ.get('ROOT', '.')).resolve()
scenarios_path = root / 'e2e' / 'scenarios.json'
test_cases_dir = root / 'docs' / 'test-cases'
...
def slugify(name):
    """Convert suite name to filename slug: 'Admin — Books Dashboard' → 'admin-books-dashboard'."""
    s = name.lower()
    s = re.sub(r'[^a-z0-9]+', '-', s)
    return s.strip('-')

def step_to_human(step):
    """Convert a Playwright step dict to a human-readable string."""
    ...
    if action == 'navigate':
        ...
        # Extract readable description from selector
...
def assertion_to_expected(assertions):
...
for suite in scenarios.get('suites', []):
    ...
    lines = [f'# {suite_name} — Test Cases\n']
    lines.append(f'> Auto-generated by e2e-updater. Last updated: {today}.\n')

    for idx, sc in enumerate(suite.get('scenarios', []), 1):
        lines.append(f'## Test Scenario {idx}: {sc["name"]}')
        lines.append(f'**ID:** {sc["id"]}\n')
        lines.append('**Steps:**')
        for si, step in enumerate(sc.get('steps', [])):
            letter = chr(ord('a') + si)
            lines.append(f'{letter}) {step_to_human(step)}')
        lines.append('')
        expected = assertion_to_expected(sc.get('assertions', []))
...
print(f'Generated {len(scenarios.get("suites", []))} test case doc(s) in {test_cases_dir}')
PY
```
