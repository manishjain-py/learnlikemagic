...
function ModeSelection({
  topic, onSelectMode, onResume, onBack, onPractice, practiceAvailable, creatingMode,
}: ModeSelectionProps) {
  ...
  useEffect(() => {
    ...
    getGuidelineSessions(topic.guideline_id)
      .catch(() => [] as GuidelineSessionEntry[])
      .then((sessionsData) => {
        if (cancelled) return;
  ...
  const incompleteTeachMe = sessions.find(
    (s) => s.mode === 'teach_me' && !s.is_complete && (s.coverage ?? 0) > 0,
  );
        ...
        <div className="selection-grid" data-testid="mode-selection" style={{ gridTemplateColumns: '1fr' }}>
          ...
          {!isRefresher && (
            ...
            >
              ...
              <span className="mode-card-sub">
                {practiceAvailable ? 'Practice what you learned' : 'No practice bank for this topic yet'}
              </span>
            </button>
          )}
          {!isRefresher && (
